    ...
    accountInfo.balances
      ...
      .map(async (coin) => {
        ...
        return {
          ...
          USD: hasPairs ? await balanceToUSD(total, coin.asset) : 0,
        };
      }),
  );

  // filter balances for minimum value set in const
  return formatedBalances.filter(balance => balance.USD > CONST.USD_BAL_MIN);
};
...
const getBudget = async (balances) => {
  ...
  return {
    BTC: await exchangeValue(totalUSD, 'USD', 'BTC'),
    USD: totalUSD,
  };
};
...
const liquidateSub = async (fundsNeeded) => {
  ...
  if (subHoldings.length > 0) {
    ...
    await asyncForEach(subHoldings, async (sub) => {
      const orderSpinner = ora({ indent: 2 }).start(
        `Need more ${CONST.TRADE_PAIR} funds, selling ${CONST.LIQUIDATE_SUB_PERCENTAGE}% of ${sub.ASSET}`,
      );
