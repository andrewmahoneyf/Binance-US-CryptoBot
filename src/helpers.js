...
export const exchangeValue = async (amount, symbol, pair) => {
  ...
  const value = await getPrice(symbol, pair);
...
/*
  Returns the value of an assets balance in USD
*/
export const balanceToUSD = async (balance, asset) => (CONST.STABLE_PAIRS.includes(asset)
  ? balance
  : exchangeValue(balance, asset, 'USD'));
